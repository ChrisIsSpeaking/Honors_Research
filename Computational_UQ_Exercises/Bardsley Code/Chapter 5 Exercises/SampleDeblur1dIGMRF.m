...
n1      = 80; %%%input(' No. of grid points = ');
h1      = 1/n1;
t       = [-1 + h1:h1:1-h1]';
N1      = 2*n1-1;
A1      = tril(ones(N1,N1))*h1;
kernel = (1/sqrt(2*pi)/sig) * exp(-(t-h1/2).^2/2/sig^2);


A1x      = A1*kernel;
err_lev1 = 2; % Percent error in data;
sigma1   = err_lev1/100 * norm(A1x) / sqrt(N1);
eta1     =  sigma1 * randn(N1,1);
b1       = A1x + eta1;
figure(1), 
  plot(t,kernel,'k',t,b1,'ko')

%  Finally, compute the least squares solution as well as the norm of the
%  perturbations of x and b.
figure(2),
  plot(t,A1\b1,'k',t,kernel,'b')
