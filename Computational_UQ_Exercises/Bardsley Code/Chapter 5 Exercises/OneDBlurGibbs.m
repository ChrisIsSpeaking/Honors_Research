...
for i = 1:nsamps-1
    ...
    %------------------------------------------------------------------
    %Computing alpha
    alphsamp(i+1) = delsamp(i+1)/lamsamp(i+1);
    %------------------------------------------------------------------
...
%Visualizing and Plotting CI for alpha
alphsamp2 = alphsamp(nburnin+1:end);
n = length(alphsamp2);
mean_val = mean(alphsamp2);
std_dev = std(alphsamp2);
sem = std_dev / sqrt(n);
df = n - 1;
confidence_level = 0.95;
t_crit = tinv(1 - (1 - confidence_level)/2, df);
...
plot( [CI_lower, CI_upper], [1,1], 'k-', 'LineWidth', 1.5);  % Black vertical line

% Plot the Mean as a Point
plot(mean_val,1, 'ro', 'MarkerSize', 4, 'MarkerFaceColor', 'r');  % Red filled circle
ylim([0.5 1.5]);                        % Set x-axis limits for clarity
xlabel('Alpha');                          % Label for y-axis
title('Alpha with 95% Confidence Interval');  % Title of the plot

hold off


% Output for (lambda,delta)-chain using sample_plot: ACF, IACT, Geweke test
lamdelalph_chain = [lamsamp(nburnin+1:end), delsamp(nburnin+1:end), alphsamp(nburnin+1:end)]';
names        = cell(2,1);
names{1}     = '\lambda';
names{2}     = '\delta';
names{3}     = '\alpha'
fignum       = 3;
[taux,acfun] = sample_plot(lamdelalph_chain,names,fignum);
