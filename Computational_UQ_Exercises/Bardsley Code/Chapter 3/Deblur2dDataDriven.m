...
ahat   = fft2(fftshift(kernel)); %This is a_s
...
%UPRE
%RegParam_fn         = @(alpha) sum((alpha^2*Utb.^2)./(dS2+alpha).^2)+...
%                        2*sigma^2*sum(dS2./(dS2+alpha));
%DP
%RegParam_fn         = @(alpha) abs(sum((alpha^2*Utb.^2)./(dS2+alpha).^2)-...
%                        n*sigma^2);
%LCurve
%RegParam_fn         = @(alpha) 
